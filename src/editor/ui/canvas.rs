...
pub fn grid_to_pixel(gx: i32, gy: i32, scroll: (i32, i32), zoom: f32, layout: &Layout) -> (i32, i32) {
    let px = ((gx - scroll.0) as f32 * 8.0 * zoom).round() as i32;
    let py = ((gy - scroll.1) as f32 * 16.0 * zoom).round() as i32;
...
pub fn draw_grid_overlay(renderer: &mut Renderer, grid: &LevelGrid, scroll: (i32, i32), zoom: f32, layout: &Layout) {
    ...
    for gy in scroll.1..(scroll.1 + ch_grid) {
        for gx in scroll.0..(scroll.0 + cw_grid) {
            let on_col = gx % 5 == 0;
            let on_row = gy % 5 == 0;
...
pub fn draw_void(renderer: &mut Renderer, grid: &LevelGrid, scroll: (i32, i32), zoom: f32, layout: &Layout) {
    let cw_grid = (layout.canvas_w as f32 / zoom).ceil() as i32;
    let ch_grid = (layout.canvas_h as f32 / zoom).ceil() as i32;
...
pub fn draw_level_boundary(renderer: &mut Renderer, grid: &LevelGrid, scroll: (i32, i32), zoom: f32, layout: &Layout) {
...
pub fn draw_cursor_highlight(renderer: &mut Renderer, mouse: &crate::mouse::MouseState, palette: &crate::editor::palette::TilePalette, select_mode: bool, zoom: f32, layout: &Layout) {
    ...
    let (px, py) = grid_to_pixel(gx, gy, (0, 0), zoom, layout);
    if select_mode {
        renderer.draw_char_scaled_pixels(px, py, '+', Color::Yellow, Color::DarkGrey, zoom);
    } else {
        let tile = palette.current();
        renderer.draw_char_scaled_pixels(px, py, tile.glyph, Color::Black, Color::White, zoom);
    }
}

pub fn draw_spawn_marker(renderer: &mut Renderer, spawn: (f32, f32), scroll: (i32, i32), zoom: f32, layout: &Layout) {
    draw_scaled_tile(renderer, spawn.0.round() as i32, spawn.1.round() as i32, '@', Color::Green, Color::Reset, scroll, zoom, layout);
}

pub fn draw_extra_spawns(renderer: &mut Renderer, spawns: &[(String, f32, f32)], scroll: (i32, i32), zoom: f32, layout: &Layout) {
...
pub fn draw_rect_preview(renderer: &mut Renderer, anchor: (i32, i32), current: (i32, i32), glyph: char, scroll: (i32, i32), zoom: f32, layout: &Layout) {
...
pub fn draw_line_preview(renderer: &mut Renderer, anchor: (i32, i32), current: (i32, i32), glyph: char, scroll: (i32, i32), zoom: f32, layout: &Layout) {
    for (gx, gy) in bresenham(anchor, current) {
        draw_scaled_tile(renderer, gx, gy, glyph, Color::Black, Color::Cyan, scroll, zoom, layout);
    }
}

pub fn draw_selection_preview(renderer: &mut Renderer, anchor: (i32, i32), current: (i32, i32), scroll: (i32, i32), zoom: f32, layout: &Layout) {
    let x0 = anchor.0.min(current.0);
    let y0 = anchor.1.min(current.1);
    let x1 = anchor.0.max(current.0);
    let y1 = anchor.1.max(current.1);
...
pub fn draw_paste_preview(renderer: &mut Renderer, clipboard: &[(i32, i32, TileRecord)], cursor: (i32, i32), flip_x: bool, flip_y: bool, rotate: i32, scroll: (i32, i32), zoom: f32, layout: &Layout) {
...
pub fn draw_erase_preview(renderer: &mut Renderer, grid_pos: (i32, i32), erase_size: usize, scroll: (i32, i32), zoom: f32, layout: &Layout) {
