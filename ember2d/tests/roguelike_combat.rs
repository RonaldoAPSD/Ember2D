...
// moved this crate below `roguelike/`, and `cargo test` runs each
...
fn a_rat_acts_at_most_once_per_player_turn_even_across_many_idle_frames() {
    ...
    // 10 input-less frames, no new player turn: with no command queued,
...
fn two_adjacent_rats_each_contribute_their_own_damage_in_the_same_resolve() {
    // **Rewritten in Step 5f** (docs/ember2d-phase5-plan.md): used to take
    // two `turn()` calls — rats published an "atk_turn_<id>"/"atk_dmg_<id>"
    // pair on turn 1 (the old every-entity-every-frame model), which
...
fn identical_input_sequences_produce_identical_state_across_independent_instances() {
    // Two independent PlayState/World/ScriptEngine stacks, driven by the
    // exact same key sequence — this is what actually catches
    // script-execution-order dependence, unlike re-running the same
    // instance twice.
    //
    // Before Step 5b (docs/ember2d-phase5-plan.md), this test's power came
    // from two instances having *different* (process-randomized) HashMap
    // iteration order — a real hash-order bug would show up as these two
    // instances diverging. Now that `World`/`ScriptState`'s maps are
    // `BTreeMap` (§5.2 H1), that source of divergence is gone by
    // construction, so this test is weaker as a hash-order probe even
    // though the system it's testing is now actually more correct. Kept
    // anyway: it still guards the command layer Phase 5e/5f add (a real bug
    // there — e.g. a command applied to the wrong actor, or dropped) would
    // still show up as h1/h2 diverging.
