...
use ember2d_sim::components::{Actor, Collider, Script, Sprite, Tag, Transform};
...
impl PlayState {
    pub(super) fn do_on_start(&mut self, world: &mut World, _events: &mut EventBus, viewport_width: usize, viewport_height: usize, persistent: &mut BTreeMap<String, rhai::Dynamic>) {
        ...
        for tile in &self.level.tiles {
            ...
            // Step 5f: only an authored `actor` (rat()/boss() in
            // examples/gen_roguelike.rs) makes a tile schedulable — see
            // TileRecord::actor's doc comment for why Ai is the only
            // controller kind ever authored this way.
            ...
            if let Some(ref graph) = tile.graph { source = ember2d_sim::graph::generate_graph(graph); }
            if !source.is_empty() {
        ...
        // Step 5f: the player is always Local(0), regardless of
        // GameplayLoop — see Actor::local's doc comment.
