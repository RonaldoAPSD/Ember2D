// color.rs — Color definitions, backed by real RGB values.
//
// Crate-root module (Step 5a, docs/ember2d-phase5-plan.md) — was
// `renderer/color.rs` until the Phase 5 workspace split needed `Color` to be
// reachable without depending on `renderer` at all. `components/sprite.rs`,
// `level.rs`, and the editor all use `Color` in ways that have nothing to do
// with GPU rendering (a tile's authored fg/bg, a save file's serialized
// value), so it belongs at the crate root, not under a rendering-specific
// module. `renderer::color` still exists as a thin re-export (see that
// file) so every existing `renderer::color::Color` path keeps compiling —
// the renderer legitimately uses `Color` too, so that re-export is honest,
// not a compatibility shim to be deleted later.
//
